...
impl InvertedIndex for InvertedIndexRam {
    fn open(path: &Path) -> std::io::Result<Self> {
        ...
        //         )
        //     })?;
        //     inverted_index.postings.push(
        //         // TODO(xzfc): avoid vector creation
        //         // PostingList { elements: posting_list.to_owned() }
        //         PostingList::from(
        //             posting_list
        //                 .iter()
        //                 .map(|i| (i.record_id, i.weight))
        //                 .collect::<Vec<_>>(),
        //         ),
        //     );
        // }
        ...
        let config_file_path = InvertedIndexMmap::index_config_file_path(path.as_ref());
        ...
        let file_path = InvertedIndexMmap::index_file_path(path.as_ref());
        match file_header.version {
            0 => {
                println!("Loading dadada");
            ...
            1 => {
                ...
                let mut pct = usize::MAX;
                for x in 0..file_header.posting_count {
                    let pct_new = 100 * x / file_header.posting_count;
                    if pct_new != pct {
                        eprintln!("Loading posting list {}%", pct_new);
                        pct = pct_new;
                    }
    ...
    fn save(&self, path: &Path) -> std::io::Result<()> {
        eprintln!("Saving inverted index to {:?}", path);

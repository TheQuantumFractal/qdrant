...
impl<T: Sized + Copy + 'static> ChunkedMmapVectors<T> {
        ...
        TKey: num_traits::cast::AsPrimitive<usize>,
    {
        ...
        let is_inserted = self.insert_many(key, vector)?;
        assert!(is_inserted, "Insertion failed");
        Ok(())
    }

    pub fn insert_many<TKey>(&mut self, start_key: TKey, vectors: &[T]) -> OperationResult<bool>
    where
        TKey: num_traits::cast::AsPrimitive<usize>,
    {
        ...
        if chunk_offset + vectors.len() > self.config.chunk_size_vectors {
            return Ok(false);
        }
        ...
        let new_len = max(self.status.len, start_key + 1);
        ...
        TKey: num_traits::cast::AsPrimitive<usize>,
    {
        ...
        Some(&chunk[chunk_offset..chunk_offset + self.config.dim])
    }
